...
/**
 * Renders the emote panel for an already-selected character. Shared by the
 * PV handler and by cancelCharacterSelect, which restores the panel that
 * changeCharacter() clears when the character select popup opens.
 */
export async function changeChar(charID: number) {
  const me = client.chars[charID];
  client.selectedEmote = -1;
  const { emotes } = client;
  const emotesList = document.getElementById("client_emo");
  emotesList.style.display = "";
  emotesList.innerHTML = ""; // Clear emote box
  const ini = await ensureCharIni(charID);
  me.side = ini.options.side;
  updateActionCommands(me.side);
  if (ini.emotions.number === 0) {
    emotesList.innerHTML =
      '<span id="emo_0" alt="unavailable" class="emote_button">No emotes available</span>';
  } else {
